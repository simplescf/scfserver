...
class NativeNotifyCallBack extends WxPayNotify
{
	public function unifiedorder($openId, $product_id)
	{
		...
		try {
			...
			SSLog::debug("unifiedorder:" . json_encode($result));
		} catch(Exception $e) {
			SSLog::error(json_encode($e));
		}
		return $result;
	}
	...
	**/
	...
	{
		SSLog::debug("call back， return xml:" . $xmlData);
		return;
	}
	...
	public function NotifyProcess($objData, $config, &$msg)
	{
		...
		//echo "处理回调";
		SSLog::debug("call back:" . json_encode($data));
		//TODO 1、进行参数校验
		if(!array_key_exists("openid", $data) ||
			!array_key_exists("product_id", $data))
		{
			$msg = "回调数据异常";
			SSLog::debug($msg  . json_encode($data));
			return false;
		}
