...
class WxPayApi
{
	...
	public static function transfers($config, $inputObj, $timeOut = 6)
	{
		...
		$inputObj->SetAppid($config->GetAppId());//公众账号ID
		$inputObj->SetMch_id($config->GetMerchantId());//商户号
		$inputObj->SetSpbill_create_ip("106.57.82.240");//终端ip	   	    
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
		
		//签名
		$inputObj->SetSign($config, false);
		...
		SSLog::info($response);
		$result = WxPayResults::Init($config, $response);
		// $result = WxPayResults::XmlToArray($response);	
		return $result;
	}
	...
	public static function shorturl($config, $inputObj, $timeOut = 6)
	{
 	 ...
 	 */
	public static function notify($config, $callback, &$msg, $xml)
	{
		//获取通知的数据
